...
import { AttireContext, OutfitColors, TurbanSuggestion } from "../types";
import { topTypeLabelForPrompt } from "../utils/attire";

/** Injected by Vite (`vite.config.ts` `define` from `.env` / `GEMINI_API_KEY`). */
...
export const getTurbanSuggestions = async (
    colors: OutfitColors,
    attireContext?: AttireContext
): Promise<TurbanSuggestion[]> => {
    const garmentLabel = topTypeLabelForPrompt(colors.topType);

    const highlightsPromptPart = colors.highlightsColor
        ? `The ${garmentLabel} also has an accent highlight color with the hex code ${colors.highlightsColor}.`
        : '';

    const contextLine = attireContext
        ? `The intended dress code / occasion is: ${attireContext}. Tailor recommendations to feel appropriate for this context.`
        : '';

    const prompt = `
        Act as an expert stylist specializing in modern Sikh men's fashion.
        A client has an outfit consisting of ${garmentLabel} for the upper garment (hex: ${colors.shirtColor}) and pants (hex: ${colors.pantsColor}).
...
function buildOutfitDescriptions(colors: OutfitColors): { upperDescription: string; lowerDescription: string } {
    let upperDescription: string;
    let lowerDescription: string;

    if (colors.topType === 'kurta') {
        upperDescription = colors.highlightsColor
            ? `a traditional knee-length kurta in hex ${colors.shirtColor} with a subtle accent detail in ${colors.highlightsColor}`
            : `a plain traditional knee-length kurta in hex ${colors.shirtColor}`;
        lowerDescription = `matching straight-cut pajama bottoms in hex ${colors.pantsColor}`;
    } else {
        const shirtTypeDescription =
            colors.topType === 'shirt' ? 'a formal shirt with a collar' : 'a casual t-shirt';
        upperDescription = colors.highlightsColor
        ...
        lowerDescription = `plain, solid-colored pants with the hex color ${colors.pantsColor}`;
    }
    return { upperDescription, lowerDescription };
}

/** Full-body render using the user's photo as identity reference + chosen palette + turban. */
export const generateStyledLookFromPortrait = async (
    file: Blob,
    colors: OutfitColors,
    turbanColor: string,
    attireContext?: AttireContext
    ...
    const imagePart = await fileToGenerativePart(file);
    const { upperDescription, lowerDescription } = buildOutfitDescriptions(colors);
    const contextLine = attireContext
        ? `The occasion is ${attireContext}; styling should feel appropriate.`
        : '';

    const prompt = `
        You are given a reference photo of a person.
        Create a new photorealistic full-body portrait where the SAME person appears (preserve their facial identity, approximate age, skin tone, and hair/beard as visible).
        ${contextLine}
        Dress them in: ${upperDescription}, and ${lowerDescription}.
        They wear a neat, stylish turban with color exactly matching hex ${turbanColor}.
        Use a clean, flattering studio or soft outdoor background. Professional fashion photography lighting.
        The result must be a single high-quality image; do not include text or watermarks.
    `;

    const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash-image',
        contents: {
            parts: [imagePart, { text: prompt.trim() }],
        },
        config: {
            responseModalities: [Modality.IMAGE],
        },
...
export const generateSikhLook = async (colors: OutfitColors, turbanColor: string): Promise<string> => {
    ...
    const prompt = `
        ...
        He is wearing ${upperDescription}, and ${lowerDescription}.
